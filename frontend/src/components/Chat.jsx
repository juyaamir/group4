...
export const Chat = ({ messages, hide2, formData }) => {
      ...
      {hide2 && (
        <div className='relative'>
            <button type='button'
              ...
              onClick={downloadIMG}><i className="fa-solid fa-download"></i>
            </button>
          <div className='border border-gray-200 mx-10 mb-10 px-4 pb-4 pt-2' ref={imgRef}>
          <Header />
          <p className='flex justify-around flex-wrap '>
            <span>
            <span className='font-bold'>Duration of Trip: </span>
            <span className='bg-yellow-200 text-sm'>{formData.start} - {formData.end}</span>
            </span>
            <span>
            <span className='font-bold'>Place: </span>
            <span className='bg-yellow-200 text-sm underline'>{formData.location}</span>
            </span>
          </p>
          <p className='p-2 Sofia text-center'>We wish you a pleasant Trip <span className='text-red-600 '>User X!</span> </p>
          {messages
